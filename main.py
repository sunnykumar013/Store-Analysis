...
class Application:
    def __init__(self,master,*args,**kwargs):
        ...
        #component
        self.heading = Label(master,text='Store Analysis',font = ('arial 40 bold'),bg = 'white')
    ...
    def ajax(self,*args,**kwargs):
        ...
        # get product info with tha id and fill it in label box
        ...
        self.bill_btn = Button(self.left,text = "Genarate Bill ",width =100,height = 2,bg ="red",fg='white',command = self.genarate_bill)
        self.bill_btn.place(x=0,y=630)


    def add_to_cart(self,*args,**kwargs):
        # gt quantity value from database
        self.quantity_value = int(self.quantity_e.get())
        if self.quantity_value > int(self.get_stock):
            tkinter.messagebox.showinfo("Error", "Not that my product in inventory ")
        else:
            #calculate the price
    ...
    def change_fun(self,*args,**kwargs):
        # get the amt given by the customer and amt generated by computer
        self.amount_given = float(self.change_e.get())
        self.our_total = sum(products_price)
    ...
    def genarate_bill(self,*args,**kwargs):
        # create the bill before generating the database
